...
from PyQt6.QtWidgets import QMainWindow, QMessageBox, QLabel, QApplication, QGraphicsBlurEffect, QWidget, QVBoxLayout, QHBoxLayout, QSpinBox, QCheckBox, QPushButton, QComboBox
...
class TenchiCardsEnhancer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.trade_thread = TradeThread(self)

        # 创建交易卡片页
        self.trade_tab = QWidget()
        self.trade_tab.setObjectName("trade_tab")
        self.init_trade_tab()
        self.tabWidget.insertTab(0, self.trade_tab, "交易卡片")
    ...
    def init_trade_tab(self):
        ...
        layout = QVBoxLayout(self.trade_tab)

        # 行：卡片选择
        row_card = QHBoxLayout()
        label_card = QLabel("卡片：")
        self.trade_card_box = SearchableComboBox(None, self)
        self.trade_card_box.setObjectName("trade_card_box")
        self.init_recipe_box(self.trade_card_box, include_pack=True, need_suffix=False)
        # 星级选择
        label_level = QLabel("星级：")
        self.trade_level_box = QComboBox()
        self.trade_level_box.setObjectName("trade_level_box")
        for i in range(16):
            self.trade_level_box.addItem(str(i))
        self.trade_level_box.setCurrentIndex(int(self.settings["交易"].get("星级", 0)))
        self.trade_level_box.currentIndexChanged.connect(self.on_trade_setting_changed)
        current_card = self.settings["交易"].get("卡片名称", "无")
        idx = 0
        for i in range(self.trade_card_box.count()):
            if self.trade_card_box.itemText(i) == current_card:
                idx = i
                break
        self.trade_card_box.setCurrentIndex(idx)
        self.trade_card_box.currentIndexChanged.connect(self.on_trade_setting_changed)
        row_card.addWidget(label_card)
        row_card.addWidget(self.trade_card_box)
        row_card.addWidget(label_level)
        row_card.addWidget(self.trade_level_box)
        layout.addLayout(row_card)

        # 行：价格（电卷）
        row_price = QHBoxLayout()
        label_price = QLabel("电卷单价：")
        self.trade_price_input = QSpinBox()
        self.trade_price_input.setRange(0, 9999999)
        self.trade_price_input.setSingleStep(1)
        self.trade_price_input.setValue(int(self.settings["交易"].get("单价", 100)))
        self.trade_price_input.setObjectName("trade_price_input")
        self.trade_price_input.valueChanged.connect(self.on_trade_setting_changed)
        row_price.addWidget(label_price)
        row_price.addWidget(self.trade_price_input)
        layout.addLayout(row_price)

        # 行：数量
        row_qty = QHBoxLayout()
        label_qty = QLabel("上架数量：")
        self.trade_qty_input = QSpinBox()
        self.trade_qty_input.setRange(1, 999)
        self.trade_qty_input.setSingleStep(1)
        self.trade_qty_input.setValue(int(self.settings["交易"].get("数量", 20)))
        self.trade_qty_input.setObjectName("trade_qty_input")
        self.trade_qty_input.valueChanged.connect(self.on_trade_setting_changed)
        row_qty.addWidget(label_qty)
        row_qty.addWidget(self.trade_qty_input)
        ...
        self.start_trade_btn = QPushButton("开始挂卡")
        self.stop_trade_btn = QPushButton("停止挂卡")
        self.stop_trade_btn.setEnabled(False)
        self.start_trade_btn.clicked.connect(self.start_trade)
        self.stop_trade_btn.clicked.connect(self.stop_trade)
        row_btns.addWidget(self.start_trade_btn)
        row_btns.addWidget(self.stop_trade_btn)
        layout.addLayout(row_btns)

    # 交易设置变更保存
    def on_trade_setting_changed(self, _value=None):
        card_name = self.trade_card_box.currentText()
        level = int(self.trade_level_box.currentText())
        price = int(self.trade_price_input.value())
        qty = int(self.trade_qty_input.value())
        only_unbound = True  # 默认仅未绑定
        self.settings.setdefault("交易", {})
        self.settings["交易"].update({
            "卡片名称": card_name,
            "星级": level,
            "单价": price,
            "数量": qty,
    ...
    def start_trade(self):
        ...
        self.send_log_message("[交易卡片] 开始挂卡...")
    ...
    def stop_trade(self):
        ...
        self.stop_trade_btn.setEnabled(False)
        self.send_log_message("[交易卡片] 已停止挂卡。")

    # 进入交易场（占位，需后续补全模板匹配）
    def enter_trade_market(self) -> bool:
        position = check_position()
        if position == "主菜单":
            self.send_log_message("[交易卡片] 尝试进入交易场（占位）。")
            return True
        else:
            change_position("主菜单")
            self.send_log_message("[交易卡片] 返回主菜单，尝试进入交易场（占位）。")
            return True

    # 执行上架（占位）
    def perform_listing(self, card_name: str, level: int, price: int, qty: int, only_unbound: bool) -> None:
        self.send_log_message(f"[交易卡片] 上架请求：卡片={card_name} 星级={level} 单价={price} 数量={qty}（仅未绑定）")
...
class TradeThread(QThread):
    ...
    def run(self):
        trade_conf = self.enhancer.settings.get("交易", {})
        card_name = trade_conf.get("卡片名称", "无")
        price = int(trade_conf.get("单价", 100))
        qty = int(trade_conf.get("数量", 20))
        only_unbound = True

        if card_name == "无" or price <= 0 or qty <= 0:
            event_manager.show_dialog_signal.emit("交易设置不完整", "请在交易卡片页设置卡片、单价和数量后再开始挂卡")
            self.enhancer.stop_trade()
            return

        if not self.enhancer.enter_trade_market():
            self.enhancer.stop_trade()
            return

        level = int(trade_conf.get("星级", 0))
        if self.enhancer.is_trading:
            self.enhancer.perform_listing(card_name, level, price, qty, only_unbound)
        self.enhancer.stop_trade()
